...
rawAssets.forEach((a) => {
  a.cashflows.forEach((cf) => {
    allCFs.push({ amount: cf.amount, date: pd(cf.date as string)! });
  });
});
...
const cagr =
  Math.round((Math.pow(netWorth / 2180000, 12 / 18) - 1) * 1000) / 10;
...
const niftyEnd = histStart * Math.pow(1.12, 18 / 12),
  midcapEnd = histStart * Math.pow(1.18, 18 / 12);
const niftyHistory = histDates.map((date, idx) => {
  const tt = idx / 17;
  return {
    date,
    value: Math.round(
...
const midcapHistory = histDates.map((date, idx) => {
  const tt = idx / 17;
  return {
    date,
    value: Math.round(
...
const fdHistory = histDates.map((date, idx) => {
  const tt = idx / 17;
  return {
    date,
...
const alphaScore = Math.min(
  100,
  Math.max(0, ((portfolioXIRR - 12) / 10) * 100),
);
...
export const phoenixData = {
  ...
  xirr: portfolioXIRR,
  cagr,
  alpha: Math.round((portfolioXIRR - 12) * 100) / 100,
  dayChange: 12400,
  dayChangePct: 0.32,
  history,
  benchmarks,
