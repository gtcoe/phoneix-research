import { Stock } from "@/types";

export const stocks: Stock[] = [
  {
    slug: "skygold",
    name: "Sky Gold & Diamonds",
    ticker: "SKYGOLD",
    exchange: "NSE",
    sector: "Jewellery / Consumer Discretionary",
    dateAnalyzed: "April 2025",
    recommendation: "buy",
    thesis:
      "45% revenue CAGR, 129% PAT CAGR over 5 years. B2B manufacturer supplying Malabar, Kalyan, Tanishq, Reliance — zero retail inventory risk with structural growth optionality.",
    mcap: "₹6,557 Cr",
    convictionScore: 9,
  },
  {
    slug: "deedev",
    name: "DEE Development Engineers",
    ticker: "DEEDEV",
    exchange: "NSE",
    sector: "Engineering / Process Piping",
    dateAnalyzed: "April 2026",
    recommendation: "buy",
    thesis:
      "India's largest integrated process piping provider. 1.8x book-to-bill order visibility, exotic materials + certification moat, nuclear and hydrogen tailwinds. Trading at 13–19x FY27 — genuinely cheap for the growth rate.",
    mcap: "₹2,827 Cr",
    convictionScore: 9,
  },
  {
    slug: "gchotels",
    name: "Grand Continent Hotels",
    ticker: "GCHOTELS",
    exchange: "NSE SME",
    sector: "Hospitality",
    dateAnalyzed: "April 2026",
    recommendation: "watch",
    thesis:
      "133% revenue growth in FY25 and 79% in H1 FY26 on an asset-light lease model. High-risk, high-reward micro-bet on South India's emerging hospitality market. Watch occupancy improvement quarterly.",
    mcap: "₹246 Cr",
    convictionScore: 5,
  },
  {
    slug: "namoewaste",
    name: "Namo eWaste Management",
    ticker: "NAMOEWASTE",
    exchange: "NSE SME",
    sector: "e-Waste / Environment Services",
    dateAnalyzed: "April 2026",
    recommendation: "watch",
    thesis:
      "50% revenue CAGR, 94% PBT growth. Early-stage e-waste recycler at the forefront of India's EPR mandate. Tiny base (₹373–417 Cr mcap) with high execution risk — strong story, needs consistent delivery.",
    mcap: "₹373–417 Cr",
    convictionScore: 5,
  },
  {
    slug: "rbz",
    name: "RBZ Jewellers",
    ticker: "RBZJEWEL",
    exchange: "NSE",
    sector: "Jewellery / Consumer Discretionary",
    dateAnalyzed: "April 2026",
    recommendation: "watch",
    thesis:
      "Best-in-class 12% EBITDA margins in jewellery, 75% promoter holding (zero pledge), antique gold moat. But guidance cut twice — a speculative bet on a difficult B2B-to-B2C transition. 3x possible in bull case.",
    mcap: "₹584 Cr",
    convictionScore: 6,
  },
  {
    slug: "efc",
    name: "EFC (I) Ltd",
    ticker: "EFCIL",
    exchange: "NSE",
    sector: "Real Estate-as-a-Service / Managed Workspaces",
    dateAnalyzed: "April 2026",
    recommendation: "buy",
    thesis:
      "India's first listed managed workspace company. 73,000+ office seats across 11 cities. Undervalued vs peers with superior profitability — asset-light model with strong expansion runway.",
    mcap: "₹2,700 Cr",
    convictionScore: 8,
  },
  {
    slug: "subros",
    name: "Subros Ltd",
    ticker: "SUBROS",
    exchange: "NSE",
    sector: "Auto Components / Thermal Management",
    dateAnalyzed: "April 2026",
    recommendation: "hold",
    thesis:
      "India's largest auto AC manufacturer. JV with Denso (20%) + Suzuki (12%). Strong EV transition bet — every car needs thermal management. Hold as an EV-cycle compounder alongside existing EPACK position.",
    mcap: "₹5,144 Cr",
    convictionScore: 7,
  },
  {
    slug: "vintage-coffee",
    name: "Vintage Coffee & Beverages",
    ticker: "VINCOFE",
    exchange: "NSE",
    sector: "FMCG / Coffee Exports",
    dateAnalyzed: "April 2026",
    recommendation: "buy",
    thesis:
      "100% capacity utilisation before new capacity commissioned — demand before supply. 74% revenue CAGR projected FY25–28 by Nuvama. The CCL Products of 2020: private label instant coffee exporter at capacity inflection with German equipment moat.",
    mcap: "₹1,981 Cr",
    convictionScore: 8,
  },
  {
    slug: "indotech",
    name: "Indo Tech Transformers",
    ticker: "INDOTECH",
    exchange: "NSE",
    sector: "Capital Goods / Power Transformers",
    dateAnalyzed: "April 2026",
    recommendation: "watch",
    thesis:
      "Solid underlying business — 7 consecutive quarters of PAT improvement, 18% EBITDA margins, 24.4% revenue CAGR. But 77% promoter pledge creates a direct governance risk unlike any other holding. Speculative only.",
    mcap: "₹1,406 Cr",
    convictionScore: 4,
  },
  {
    slug: "websol",
    name: "Websol Energy System",
    ticker: "WEBELSOLAR",
    exchange: "NSE",
    sector: "Renewable Energy / Solar Manufacturing",
    dateAnalyzed: "April 2026",
    recommendation: "buy",
    thesis:
      "India's pioneering pure-play solar cell manufacturer with a 3-decade track record. Falta SEZ location provides cost advantages. Best solar manufacturing play for a 3x goal with strong policy tailwinds.",
    mcap: "₹3,200 Cr",
    convictionScore: 8,
  },
  {
    slug: "cellecor",
    name: "Cellecor Gadgets Ltd",
    ticker: "CELLECOR",
    exchange: "NSE SME",
    sector: "Consumer Electronics",
    dateAnalyzed: "April 2026",
    recommendation: "watch",
    thesis:
      "Budget electronics brand (TVs, ACs) with ₹1,026 Cr revenue and proven doubling track record. But 3% PAT margins have never improved in 3 years, SME board liquidity is thin, FCCB dilution looming. Base case only 1.7–1.9x — challenging path to 3x.",
    mcap: "₹671 Cr",
    entryPrice: "₹32",
    horizon: "2–3 years",
    convictionScore: 3,
  },
  {
    slug: "anant-raj",
    name: "Anant Raj Ltd",
    ticker: "ANANTRAJ",
    exchange: "NSE",
    sector: "Real Estate / Data Centres",
    dateAnalyzed: "April 2026",
    recommendation: "hold",
    thesis:
      "Real estate + data centre pivot. 320 acres debt-free Delhi-NCR land bank. DC capacity target 1GW by FY30. Structural re-rating play as DC revenue scales.",
    mcap: "₹12,900 Cr",
    convictionScore: 8,
  },
  {
    slug: "borana-weaves",
    name: "Borana Weaves Ltd",
    ticker: "BORANA",
    exchange: "BSE",
    sector: "Textiles / Synthetic Fabric",
    dateAnalyzed: "April 2026",
    recommendation: "buy",
    thesis:
      "9x revenue in 4 years (₹42Cr→₹367Cr), 20% EBITDA margins consistent. Best risk/reward synthetic fabric manufacturer. BSE SME, high growth, reasonable valuations.",
    mcap: "₹1,013 Cr",
    convictionScore: 8,
  },
  {
    slug: "rpsg-ventures",
    name: "RPSG Ventures Ltd",
    ticker: "RPSGVENT",
    exchange: "NSE",
    sector: "Holding Company / Diversified",
    dateAnalyzed: "April 2026",
    recommendation: "buy",
    thesis:
      "Buying ₹14,000–25,000 Cr NAV for ₹3,094 Cr mcap — 80% discount. Firstsource + IPL + FMCG brands. Discount narrowing to 55–60% = 3x.",
    mcap: "₹3,094 Cr",
    convictionScore: 8,
  },
  {
    slug: "taril",
    name: "TARIL (Transformers & Rectifiers)",
    ticker: "TARIL",
    exchange: "NSE",
    sector: "Capital Goods / Power Transformers",
    dateAnalyzed: "April 2026",
    recommendation: "hold",
    thesis:
      "₹5,005 Cr order book (2.1x revenue). ₹23,000 Cr inquiry pipeline. Best value power transformer stock after -50% correction from peak.",
    mcap: "₹3,500 Cr",
    convictionScore: 7,
  },
  {
    slug: "zaggle",
    name: "Zaggle Prepaid Ocean Services",
    ticker: "ZAGGLE",
    exchange: "NSE",
    sector: "Fintech / SaaS",
    dateAnalyzed: "April 2026",
    recommendation: "hold",
    thesis:
      "India's Edenred thesis. 50M+ prepaid cards, 1,500+ enterprise clients. At entry price of ₹225 even base case delivers 2x.",
    mcap: "₹3,500 Cr",
    convictionScore: 7,
  },
  {
    slug: "ideaforge",
    name: "ideaForge Technology",
    ticker: "IDEAFORGE",
    exchange: "NSE",
    sector: "Aerospace & Defence / Drones",
    dateAnalyzed: "April 2026",
    recommendation: "buy",
    thesis:
      "India's #1 drone manufacturer, 50% market share, 18-year head start in defense procurement. Pure-play UAV — highest binary risk/reward in the session.",
    mcap: "₹2,357 Cr",
    convictionScore: 7,
  },
  {
    slug: "nurture-well",
    name: "Nurture Well Industries",
    ticker: "NWIL",
    exchange: "BSE",
    sector: "FMCG / Food Products",
    dateAnalyzed: "April 2026",
    recommendation: "buy",
    thesis:
      "57% revenue growth, 10.4% PAT margins, 28% ROE, zero long-term debt at ~8x trailing PE. Cheapest high-growth FMCG in the watchlist.",
    mcap: "₹832 Cr",
    convictionScore: 7,
  },
  {
    slug: "quality-power",
    name: "Quality Power Electrical",
    ticker: "QPOWER",
    exchange: "NSE",
    sector: "Capital Goods / Power Equipment",
    dateAnalyzed: "April 2026",
    recommendation: "watch",
    thesis:
      "HVDC & FACTS equipment monopoly in India. Non-replicable moat. Outstanding business but IPO premium makes 3x difficult at current price.",
    mcap: "₹9,835 Cr",
    convictionScore: 6,
  },
  {
    slug: "capacite",
    name: "Capacit'e Infraprojects",
    ticker: "CAPACITE",
    exchange: "NSE",
    sector: "Construction / EPC",
    dateAnalyzed: "April 2026",
    recommendation: "hold",
    thesis:
      "EPC for high-rise buildings at 10x PE. Real merit but governance concerns (low promoter holding, auditor flags on receivables). Hold existing position, do not add.",
    mcap: "₹1,950 Cr",
    convictionScore: 5,
  },
  {
    slug: "rbm-infracon",
    name: "RBM Infracon Ltd",
    ticker: "RBMINFRA",
    exchange: "NSE SME",
    sector: "Engineering / Oil & Gas EPC",
    dateAnalyzed: "April 2026",
    recommendation: "hold",
    thesis:
      "₹4,531 Cr order book on ₹448 Cr mcap (10x coverage). ONGC 15-year contract. Semiconductor LOI optionality. Speculative hold.",
    mcap: "₹448 Cr",
    convictionScore: 5,
  },

  // ── May 2026 additions ───────────────────────────────────────────────────
  {
    slug: "eco-recycling",
    name: "Eco Recycling Ltd (Ecoreco)",
    ticker: "ECORECO",
    exchange: "BSE",
    sector: "e-Waste Management",
    dateAnalyzed: "May 2026",
    recommendation: "watch",
    thesis:
      "India's only BSE-listed e-waste player with a 99% PAT CAGR over 5 years and a regulatory moat that strengthens over time. Working capital deterioration and revenue lumpiness limit conviction to a small speculative allocation until FY27 Q1 validates sustainability.",
    mcap: "₹830 Cr",
    convictionScore: 5,
  },
  {
    slug: "genus-power",
    name: "Genus Power Infrastructures",
    ticker: "GENUSPOWER",
    exchange: "NSE",
    sector: "Capital Goods / Smart Metering",
    dateAnalyzed: "May 2026",
    recommendation: "hold",
    thesis:
      "India's #1 smart meter company with 27% market share and ₹27,000 Cr order book. AMISP + RDSS tailwinds. GIC USD 2B platform partnership. Hold and possibly add small on dips.",
    mcap: "₹8,300 Cr",
    convictionScore: 6,
  },
  {
    slug: "krsnaa-diagnostics",
    name: "Krsnaa Diagnostics",
    ticker: "KRSNAA",
    exchange: "NSE",
    sector: "Healthcare / Diagnostics",
    dateAnalyzed: "May 2026",
    recommendation: "watch",
    thesis:
      "PPP diagnostics model across 18 states with government-contracted revenue. CFO and 3 directors resigned in 4 months, promoter holding at 27.1%. Governance risk unresolved — watchlist only until clarity emerges.",
    mcap: "₹1,900 Cr",
    convictionScore: 3,
  },
  {
    slug: "meta-infotech",
    name: "Meta Infotech Ltd",
    ticker: "METAINF",
    exchange: "BSE",
    sector: "Cybersecurity",
    dateAnalyzed: "May 2026",
    recommendation: "watch",
    thesis:
      "BSE SME cybersecurity (SASE, Cloud Security). Promoters sold 26% of stake within 2 months of IPO — used IPO as liquidity exit. On ASM list. Stock -48% from IPO price. Governance disqualifies as a serious holding.",
    mcap: "₹158 Cr",
    convictionScore: 2,
  },
  {
    slug: "muthoot-microfin",
    name: "Muthoot Microfin",
    ticker: "MUTHOOTMF",
    exchange: "NSE",
    sector: "NBFC / Microfinance",
    dateAnalyzed: "May 2026",
    recommendation: "buy",
    thesis:
      "Most compelling recovery + compounding story in the MFI sector. 3x PAT growth from ₹150 Cr (FY26) to ₹450–530 Cr (FY28) mechanically driven by credit cost normalisation. Part of Muthoot Pappachan Group. Buy ₹25–40K as a medium-conviction position.",
    mcap: "₹3,185 Cr",
    convictionScore: 7,
  },
  {
    slug: "sterlite-tech",
    name: "Sterlite Technologies (STL)",
    ticker: "STLTECH",
    exchange: "NSE",
    sector: "Technology / Optical Fibre",
    dateAnalyzed: "May 2026",
    recommendation: "watch",
    thesis:
      "Excellent quality optical fibre + AI infrastructure business, but stock already returned +614% from April 2025 lows. At ₹14,390 Cr mcap with ₹56 Cr PAT = 257x PE, 3x math requires 30% PAT growth. Do not buy at current prices.",
    mcap: "₹14,390 Cr",
    convictionScore: 3,
  },
  {
    slug: "advait-energy",
    name: "Advait Energy Transitions Ltd",
    ticker: "ADVAIT",
    exchange: "NSE",
    sector: "Energy / Power Transmission",
    dateAnalyzed: "May 2026",
    recommendation: "buy",
    thesis:
      "Multi-vertical energy transition story on top of a proven ₹680 Cr+ PTS business. Building 5 new revenue streams (BESS BOO, Electrolyser, Fuel Cell, Carbon Advisory). 3x by FY28 if BESS + Electrolyser contribute ₹100+ Cr PAT. Promoter 66.8% with zero pledge. Speculative buy ₹15–20K after FY26 results.",
    mcap: "₹2,000 Cr",
    convictionScore: 6.5,
  },
  {
    slug: "c2c-advanced-systems",
    name: "C2C Advanced Systems Ltd",
    ticker: "C2C",
    exchange: "NSE",
    sector: "Defence / C4ISR",
    dateAnalyzed: "May 2026",
    recommendation: "buy",
    thesis:
      "Best 3x math in India's defence sector: 17.5x PE vs peers at 75–107x, with the HIGHEST PAT margins (31% vs 12–26%). C4ISR + Anti-Drone + AI/ML Defence. Post-Operation Sindoor demand surge. PE discount is entirely SME-listing driven — closes fast on mainboard migration. Kacholia 3.51% holding.",
    mcap: "₹750 Cr",
    convictionScore: 8,
  },
  {
    slug: "sahasra-electronic",
    name: "Sahasra Electronic Solutions Ltd",
    ticker: "SAHASRA",
    exchange: "NSE",
    sector: "Electronics / ESDM",
    dateAnalyzed: "March 2026",
    recommendation: "buy",
    thesis:
      "Cheapest PE (33x) vs Dixon 130x, Amber 176x, Kaynes 243x — for HIGHER revenue growth (65%+ CAGR). Two businesses: EMS + Semiconductor subsidiary (option value not priced in). Mukul Agrawal 2.76% fresh Q4 FY26 add. NSE Emerge SME — illiquidity risk offset by valuation gap.",
    mcap: "₹700 Cr",
    convictionScore: 7.5,
  },
  {
    slug: "satin-creditcare",
    name: "Satin Creditcare Network Ltd",
    ticker: "SATIN",
    exchange: "NSE",
    sector: "NBFC / MFI",
    dateAnalyzed: "May 2026",
    recommendation: "watch",
    thesis:
      "3rd largest NBFC-MFI by AUM (₹15,275 Cr) trading at P/B 0.83x (below book). 15 consecutive profitable quarters. Re-rating from 0.83x → 1.5x alone = 80% return. Watchlist — wait for May 12 FY26 earnings call confirmation before entry.",
    mcap: "₹2,350 Cr",
    convictionScore: 6,
  },
  {
    slug: "vmarc-india",
    name: "V-Marc India Ltd",
    ticker: "VMARCIND",
    exchange: "NSE",
    sector: "Cables & Wires",
    dateAnalyzed: "April 2026",
    recommendation: "watch",
    thesis:
      "Fast-growing North India cables manufacturer: 7x revenue in 4 years to ₹1,251 Cr TTM. Ashish Kacholia 2.71% stake. But at 43x PE on ₹61 Cr PAT = pricing in perfection. Wait for 30–40% correction to ₹1,600–1,800 Cr mcap before entry. NSE Emerge SME listing.",
    mcap: "₹2,621 Cr",
    convictionScore: 5.5,
  },
  {
    slug: "valiant-communications",
    name: "Valiant Communications",
    ticker: "VALIANT",
    exchange: "NSE",
    sector: "Defence / Telecom",
    dateAnalyzed: "May 2026",
    recommendation: "watch",
    thesis:
      "Telecom transmission + cybersecurity + defence company. ₹52 Cr revenue at ₹1,050 Cr mcap = 20x Price/Sales. Theme (defence + quantum) is valid but the vehicle is mispriced for a 3x strategy. Better-conviction names exist in the portfolio.",
    mcap: "₹1,050 Cr",
    convictionScore: 2,
  },
];

export function getStockBySlug(slug: string): Stock | undefined {
  return stocks.find((s) => s.slug === slug);
}
